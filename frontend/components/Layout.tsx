...
const Layout = ({
  ...
  const handleLogout = async () => {
    try {
      ...
      setIsLoggedIn(false);
      if (isUser) router.push("/user/login");
      else if (isVenue) router.push("/venue/venueLogin");
      else router.push("/");
    } catch (error) {
  ...
  return (
    <div>
                ...
                {isLoggedIn && (
                  <li>
                    <Link
                      ...
                      className={getNavLinkClass(
                        isUser ? "/user/dashboard" : "/venue/venueDashboard"
                      )}
                    >
                      Dashboard
                    </Link>
                ...
                <li>
                  {isLoggedIn ? (
                    <Link
                      ...
                      className={getNavLinkClass(isUser ? "/user/profile" : "/venue/venueProfile")}
                    >
                      Profile
                    </Link>
                  ) : (
      ...
      <main className="pt-2 pb-16 pt-16">{children}</main>{" "}
      {/* Add padding bottom */}
      <footer className="fixed bottom-0 left-0 z-20 w-full p-4 bg-white border-t border-gray-200 shadow md:flex md:items-center md:justify-between md:p-6 dark:bg-gray-800 dark:border-gray-600">
        <span className="text-sm text-gray-500 sm:text-center dark:text-gray-400">
          © 2024 Meal app™ . All Rights Reserved.
        </span>
