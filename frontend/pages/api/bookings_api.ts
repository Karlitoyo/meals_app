...
export const fetcher = async <T>(
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_FRONTEND_URL}${url}`, {
      headers: {
  ...
  export const getAvailability = async (venueId: number): Promise<Availability[]> =>
    fetcher(`/availability?venueId=${venueId}`);
